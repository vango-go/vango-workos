...
func TestMiddleware_ExpiredBeyondLeewayRefreshEnabledSuccess(t *testing.T) {
	key := mustRSAKey(t)
	expiredClaims := baseClaims()
	...
	refreshedClaims := baseClaims()
	refreshedClaims.ExpiresAt = jwt.NewNumericDate(time.Now().Add(10 * time.Minute))
	refreshedClaims.Email = "refreshed_beyond@example.com"
	refreshedToken := signRS256Token(t, key, "kid-1", refreshedClaims)
	...
	if gotIdentity.Email != "refreshed_beyond@example.com" {
		t.Fatalf("Email = %q, want %q", gotIdentity.Email, "refreshed_beyond@example.com")
	}
...
func TestMiddleware_MissingExp_DoesNotRefresh_ClearsCookie(t *testing.T) {
	key := mustRSAKey(t)
	claims := baseClaims()
